...
import numpy as np
from model import DynamicMemoryNetwork
#from aa1_data_util.\
...
tf.app.flags.DEFINE_integer("num_classes",369,"number of label") #3 ADDITIONAL TOKEN: _GO,_END,_PAD
...
tf.app.flags.DEFINE_integer("decay_steps", 500, "how many steps before decay learning rate.") #6000批处理的大小 32-->128
...
tf.app.flags.DEFINE_integer("sequence_length",1024,"max sentence length") #100
...
tf.app.flags.DEFINE_string("traning_data_path","../sogou_data/sogou.multi.train.txt","path of traning data.") #O.K.train-zhihu4-only-title-all.txt-->training-data/test-zhihu4-only-title.txt--->'training-data/train-zhihu5-only-title-multilabel.txt'
tf.app.flags.DEFINE_string("word2vec_model_path","../sogou_data/sogou.desc.word2vec.file","word2vec's vocabulary and vectors") #zhihu-word2vec.bin-100-->zhihu-word2vec-multilabel-minicount15.bin-100
...
def main(_):
    ...
    if 1==1:
        ...
        if FLAGS.multi_label_flag:
            FLAGS.traning_data_path='../sogou_data/sogou.multi.train.txt' #change this line if want to train in a small dataset. e.g. dataset from 'test-zhihu6-title-desc.txt'
    ...
    with tf.Session(config=config) as sess:
        ...
        for epoch in range(curr_epoch,FLAGS.num_epochs):
            ...
            for start, end in zip(range(0, number_of_training_data, batch_size),range(batch_size, number_of_training_data, batch_size)):
                ...
                if FLAGS.batch_size!=0 and (start%(FLAGS.validate_step*FLAGS.batch_size)==0): #(epoch % FLAGS.validate_every) or  if epoch % FLAGS.validate_every == 0:
                    ...
                    print("dynamic_memory_network[use_gated_gru=False,num_pass=2].validation.part. previous_eval_loss:", previous_eval_loss,";current_eval_loss:", eval_loss,";pre_acc:",pre_acc,";eval_acc:",eval_acc)
                    if eval_loss > previous_eval_loss: #if loss is not decreasing
                        ...
                        if eval_loss<best_eval_loss and eval_acc>pre_acc:
                            print("dynamic_memory_network[use_gated_gru=False,num_pass=2]==>going to save the model.eval_loss:",eval_loss,";best_eval_loss:",best_eval_loss)
        ...
        print("final test Loss:%.3f\ttest Accuracy: %.3f" % (test_loss,test_acc))
    pass
...
def do_eval(sess,model,evalX,evalY,batch_size,vocabulary_index2word_label,eval_decoder_input=None):
    ...
    eval_loss,eval_acc,eval_counter,eval_loss_count=0.0,0.0,0,0
    for start,end in zip(range(0,number_examples,batch_size),range(batch_size,number_examples,batch_size)):
        ...
        for i in range(len(logits)):
            label_list_top5 = get_label_using_logits(logits[i], vocabulary_index2word_label)
            curr_eval_acc=calculate_accuracy(list(label_list_top5), evalY[start:end][i], eval_counter, vocabulary_index2word_label)
            eval_acc,eval_counter=eval_acc+curr_eval_acc,eval_counter+1
        eval_loss, eval_loss_count=eval_loss+curr_eval_loss, eval_loss_count+1        

        #eval_loss,eval_acc,eval_counter=eval_loss+curr_eval_loss,eval_acc+curr_eval_acc,eval_counter+1
    return eval_loss/float(eval_loss_count),eval_acc/float(eval_counter)
...
# calcuate accuracy
def calculate_accuracy(labels_predicted, labels,eval_counter, vocabulary_index2word_label=None):
    label_nozero=[]
    #print("labels:",labels)
    labels=list(labels)
    ...
    #if eval_counter<2:
    #if eval_counter>=0 and vocabulary_index2word_label != None:
        #print("labels_predicted:",labels_predicted," ;labels_nozero:",label_nozero)
        #printLabel(labels_predicted, label_nozero, eval_counter, vocabulary_index2word_label)
    count = 0
...
def printLabel(labels_predicted, label_nozero, eval_counter, vocabulary_index2word_label):
    label_p = []
    label_t = []
    for index in labels_predicted:
        label_p.append(vocabulary_index2word_label[index])
    for index in label_nozero:
         label_t.append(vocabulary_index2word_label[index])
    print("test:",eval_counter,"labels_p:",label_p," ;labels_t:",label_t)
